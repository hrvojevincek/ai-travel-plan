    ...
    days: [
      {
        ...
        activities: [
          ...
          { name: "Cervejaria Ramiro", type: "food", orderIndex: 3, estimatedCost: 35 },
          { name: "Alfama walk", type: "sightseeing", orderIndex: 4 },
          { name: "Tram 28", type: "sightseeing", orderIndex: 5 },
          { name: "Tasca Dinner", type: "food", orderIndex: 6, estimatedCost: 25 },
        ],
      },
    ],
  };
...
describe("swapActivity", () => {
  ...
  it("returns output that updateActivity accepts (caller contract)", async () => {
    ...
    await updateActivity(db, target.id, { ...result, orderIndex: target.orderIndex });
